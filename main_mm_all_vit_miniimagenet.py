...
import torch
import numpy as np
from dataset_img import MiniImageNetMMDataset,MiniImageNetDatasetArgs
...
from torch.utils.data import DataLoader
WANDB = os.environ.get("WANDB_DISABLED", "0") != "1"
...
def log(*args, **kwargs):
    ...
    if _state["logfile"]:
        ...
        _builtin_print(*args, file=_state["logfile"], **kwargs)
...
if __name__ == "__main__":
    _state["logfile"] = open("mm_train.log", "w")
    ...
    img_size = 84
    patch_size = int(sys.argv[33]) if len(sys.argv) > 33 else 7
    enc_depth = int(sys.argv[34]) if len(sys.argv) > 34 else 12
    enc_heads = int(sys.argv[35]) if len(sys.argv) > 35 else 6
    enc_dropout = float(sys.argv[36]) if len(sys.argv) > 36 else 0.1
    enc_iterations = int(sys.argv[37]) if len(sys.argv) > 37 else 30000
    enc_num_classes = int(sys.argv[38]) if len(sys.argv) > 38 else 64
    no_repeats = False
    ...
    ckpt_path_enc = sys.argv[39] if len(sys.argv) > 39 else f"{root}/outs_encoder_vit_miniimagenet/K_{enc_num_classes}_eps_0.0_alpha_0.0_augment_False_patch_size_{patch_size}_output_dim_{D2}_depth_{enc_depth}_heads_{enc_heads}_dropout_{enc_dropout}_niter_{enc_iterations}/seed_0/ckpt_{enc_iterations-1}.pt"
   ...
   # Initialize wandb
    prefix = f"./outs_torch_icml_rebuttal/MiniImageNet_K2_{K2}_n_img_{n_img_per_class}_eps_{eps0}_a_{alpha0}_aug_{augment}_out_{D2}_MM_K1_{K1}_L1_{L1}_L2_{L2}_a1_{alpha1}_a2_{alpha2}_B{B}_pB{p_B}_pC{p_C}_eps1_{eps1}_eps2_{eps2}_encoder_{encoder}_freeze_layers{freeze_layers}_freeze_encoder{freeze_encoder}"
    if WANDB:
    ...
    enc_total = _count(model.encoder)
    enc_train = _count_trainable(model.encoder)
    proj_total = _count(model.projector)
    proj_train = _count_trainable(model.projector)
    dec_total = sum(p.numel() for n, p in model.named_parameters()
                    if not n.startswith("encoder.") and not n.startswith("projector."))
    dec_train = sum(p.numel() for n, p in model.named_parameters()
                    if p.requires_grad and not n.startswith("encoder.") and not n.startswith("projector."))
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Param count] Encoder  : {enc_total:>10,} total | {enc_train:>10,} trainable")
    print(f"[Param count] Projector: {proj_total:>10,} total | {proj_train:>10,} trainable")
    print(f"[Param count] Decoder  : {dec_total:>10,} total | {dec_train:>10,} trainable")
    print(f"[Param count] ALL      : {total_params:>10,} total | {trainable_params:>10,} trainable")
    assert total_params == enc_total + proj_total + dec_total, \
        f"Param mismatch: {total_params} != {enc_total} + {proj_total} + {dec_total}"
    if WANDB:
        wandb.config.update({
            "total_params": total_params, "trainable_params": trainable_params,
            "encoder_params": enc_total, "encoder_trainable": enc_train,
            "projector_params": proj_total, "projector_trainable": proj_train,
            "decoder_params": dec_total, "decoder_trainable": dec_train,
        })
    ...
    dataset_args = MiniImageNetDatasetArgs(K=K2, eps=eps0, alpha=alpha0, augment=augment)
    ...
    train_dataset = MiniImageNetMMDataset(dataset_args, mus_label_m1=mus_label_m1, mus_class_m1=mus_class_m1, mus_label_m2=mus_label_m2, labels_class_m2=labels_class_m2, mapping_m2_to_m1=mapping_m2_to_m1, N=N,S=batch_size,eps1=eps1,eps2=eps2, P1 = P1, P2 = P2, B=B, p_B = p_B, p_C = p_C, no_repeats = no_repeats, datasize=niters)
