...
def compute_ih_strength(seq_labels, attn_weights):
    # Returns: [[L0_H0, L0_H1...], [L1_H0, L1_H1...]]
    ...
    bsz, n_heads, seq_len, _ = attn_weights[0].shape
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            # Shape: (batch, seq, seq)
            head_attn_weights = attn_weights[i][:, h, :, :]
            
            attn_correct = [[head_attn_weights[n][-1][3*k+2] for k in m] for n, m in enumerate(matches)]
            attn_incorrect = [[head_attn_weights[n][-1][3*k+2] for k in m] for n, m in enumerate(unmatches)]
            
            score_correct = [sum(attn_correct[n])/len(attn_correct[n]) if len(attn_correct[n]) > 0 else 0 for n in range(len(matches))]
            score_incorrect = [sum(attn_incorrect[n])/len(attn_incorrect[n]) if len(attn_incorrect[n]) > 0 else 0 for n in range(len(unmatches))]
            
            ih_strength = [score_correct[n] - score_incorrect[n] for n in range(len(score_correct))]
            ih_strength = sum(ih_strength)/len(ih_strength) if len(ih_strength) > 0 else 0
            layer_scores.append(ih_strength)
...
def compute_TILA(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            ...
            attn_correct = [[head_attn_weights[n][-1][3*k+2] for k in m] for n, m in enumerate(matches)]
            score_correct = [sum(attn_correct[n])/len(attn_correct[n]) if len(attn_correct[n]) > 0 else 0 for n in range(len(matches))]
            score_correct = sum(score_correct)/len(score_correct) if len(score_correct) > 0 else 0
            layer_scores.append(score_correct)
        scores.append(layer_scores)
    return scores
...
def compute_TIIA_m1(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn_weights = attn_weights[i][:, h, :, :]
            # 3*k is the first item in the pair (Input)
            attn_correct = [[head_attn_weights[n][-1][3*k] for k in m] for n, m in enumerate(matches)]
            score_correct = [sum(attn_correct[n])/len(attn_correct[n]) if len(attn_correct[n]) > 0 else 0 for n in range(len(matches))]
            score_correct = sum(score_correct)/len(score_correct) if len(score_correct) > 0 else 0
            layer_scores.append(score_correct)
        scores.append(layer_scores)
...
def compute_TIIA_m2(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn_weights = attn_weights[i][:, h, :, :]
            # 3*k+1 is the second item (Separator/Middle)
            attn_correct = [[head_attn_weights[n][-1][3*k+1] for k in m] for n, m in enumerate(matches)]
            score_correct = [sum(attn_correct[n])/len(attn_correct[n]) if len(attn_correct[n]) > 0 else 0 for n in range(len(matches))]
            score_correct = sum(score_correct)/len(score_correct) if len(score_correct) > 0 else 0
            layer_scores.append(score_correct)
        scores.append(layer_scores)
...
def compute_TLA(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn_weights = attn_weights[i][:, h, :, :]
            tla = [[head_attn_weights[n][-1][3*k+2] for k in range(n_pairs)] for n in range(bsz)]
            tla = [sum(tla[n]) for n in range(bsz)]
            tla = sum(tla)/len(tla) if len(tla) > 0 else 0
            layer_scores.append(tla)
        scores.append(layer_scores)
    return scores
...
def compute_prev_1_attn(attn_weights):
    ...
    for i in range(n_layers):
        layer_scores = []
        for h in range(n_heads):
            attn = attn_weights[i][:, h, :, :] # (bsz, seq, seq)
            attn_to_prev = attn.diagonal(offset=-1, dim1=-2, dim2=-1)
            attn_to_prev = attn_to_prev.mean(dim=-1).mean(dim=-1)
            layer_scores.append(attn_to_prev.item())
        prev_1_attns.append(layer_scores)
    return prev_1_attns
...
def compute_prev_2_attn(attn_weights):
    ...
    for i in range(n_layers):
        layer_scores = []
        for h in range(n_heads):
            attn = attn_weights[i][:, h, :, :]
            attn_to_prev = attn.diagonal(offset=-2, dim1=-2, dim2=-1)
            attn_to_prev = attn_to_prev.mean(dim=-1).mean(dim=-1)
            layer_scores.append(attn_to_prev.item())
        prev_2_attns.append(layer_scores)
    return prev_2_attns
...
def flatten_metrics(metric_name, metric_data):
    """Helper to flatten [[L0H0, L0H1], [L1H0...]] into dict for wandb"""
    data_dict = {}
    for l, layer_data in enumerate(metric_data):
        for h, head_val in enumerate(layer_data):
            data_dict[f"{metric_name}_L{l}_H{h}"] = head_val
    return data_dict


def format_metric_string(metric_name, metric_data):
    """Helper to create a string summary for printing"""
    s = ""
    for l, layer_data in enumerate(metric_data):
        for h, head_val in enumerate(layer_data):
            s += f", {metric_name}_L{l}_H{h}: {head_val:.4f}"
    return s
    ...
    for epoch in range(n_epochs):
        ...
        for n, batch in tqdm(enumerate(train_loader), total=niters):
            ...
            if global_iter%print_every==0:
                ...
                log_dict = {
                    "Epoch": epoch, "Iteration": n, "global_iter": global_iter,
                    "Train_Loss": loss, "Train_Accuracy": acc, 
                    "Test_Loss": loss_test, "Test_Accuracy": acc_test,
                    "IW_Loss": loss_iw, "IW_Accuracy": acc_iw
                }
                print_str = f"Epoch {epoch}, Iteration {n}: Train loss: {loss:.4f}, Train acc: {acc:.4f}, Test loss: {loss_test:.4f}, Test acc: {acc_test:.4f}"
                if progress_measure:
                    ...
                    log_dict.update({
                        "IC_Loss": loss_ic, "IC_Accuracy": acc_ic,
                        "IC2_Loss": loss_ic2, "IC2_Accuracy": acc_ic2,
                        "prob_icl_labels": prob_icl_labels_ic
                    })
                    log_dict.update(flatten_metrics("IH_strength", ih_strengths_ic))
                    log_dict.update(flatten_metrics("TIIA_m1", tiias_m1_ic))
                    log_dict.update(flatten_metrics("TIIA_m2", tiias_m2_ic))
                    log_dict.update(flatten_metrics("TILA", tilas_ic))
                    log_dict.update(flatten_metrics("TLA", tlas_ic))
                    log_dict.update(flatten_metrics("prev_1_attn", prev_1_attns_ic))
                    log_dict.update(flatten_metrics("prev_2_attn", prev_2_attns_ic))
                    print_str += f", IC loss: {loss_ic:.4f}, IC acc: {acc_ic:.4f}, IC2 loss: {loss_ic2:.4f}, IC2 acc: {acc_ic2:.4f}"
                    print_str += format_metric_string("IH", ih_strengths_ic)
                    print_str += f", prob_icl: {prob_icl_labels_ic:.4f}"
                else:
                    loss_ic, acc_ic = metric_ic
                    loss_ic2, acc_ic2 = metric_ic2
                    log_dict.update({"IC_Loss": loss_ic, "IC_Accuracy": acc_ic, "IC2_Loss": loss_ic2, "IC2_Accuracy": acc_ic2})
                    print_str += f", IC loss: {loss_ic:.4f}, IC acc: {acc_ic:.4f}, IC2 loss: {loss_ic2:.4f}, IC2 acc: {acc_ic2:.4f}, IW loss: {loss_iw:.4f}, IW acc: {acc_iw:.4f}"
   ...
   # Initialize wandb
    ...
    if WANDB:
        wandb.init(project="ICL_torch",
                ...
                tags=["rebuttal","multi_layer_head_analysis"]
                )
    ...
    if not early_fusion:
        ...
        # Check if there are only missing keys and they all start with 'projector'
        missing_keys = load_result.missing_keys
        unexpected_keys = load_result.unexpected_keys
        if unexpected_keys:
            print(f"Warning: Unexpected keys found: {unexpected_keys}")
            print("Weights NOT loaded due to unexpected keys")
        else:
            # Check if all missing keys start with 'projector'
            if missing_keys and all(key.startswith('projector') for key in missing_keys):
                print(f"Missing keys (all projector-related): {missing_keys}")
                print("Loaded weights from checkpoint", weights)
            elif not missing_keys:
                print("Loaded weights from checkpoint", weights)
            else:
                non_projector_missing = [key for key in missing_keys if not key.startswith('projector')]
                print(f"Warning: Non-projector missing keys found: {non_projector_missing}")
                print("Weights NOT loaded due to non-projector missing keys")
                print("Loaded weights from checkpoint", weights)
    if freeze_layers:
