...
def compute_ih_strength(seq_labels, attn_weights):
    # Compute the attention difference the target paid to the correct labels and incorrect labels
    ...
    for i in range(n_layers):
        # attn_weights[i] shape: [Batch, Heads, Seq, Seq]
        n_heads = attn_weights[i].shape[1]
        layer_scores = []
        
        for h in range(n_heads):
            head_attn = attn_weights[i][:, h, :, :] # Shape: [Batch, Seq, Seq]
            
            attn_correct = [[head_attn[n][-1][2*i+1] for i in m] for n,m in enumerate(matches)]
            attn_incorrect = [[head_attn[n][-1][2*i+1] for i in m] for n,m in enumerate(unmatches)]
...
def compute_TIIA(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn = attn_weights[i][:, h, :, :]
            attn_correct = [[head_attn[n][-1][2*i] for i in m] for n,m in enumerate(matches)]
            score_correct = [sum(attn_correct[k])/len(attn_correct[k]) if len(attn_correct[k]) > 0 else 0 for k in range(len(matches))]
            score_correct = sum(score_correct)/len(score_correct)
            layer_scores.append(score_correct)
...
def compute_TLA(seq_labels, attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn = attn_weights[i][:, h, :, :]
            tla = [[head_attn[n][-1][2*j+1] for j in range(n_pairs)] for n in range(bsz)]
...
def compute_prev_1_attn(attn_weights):
    ...
    for i in range(n_layers):
        ...
        for h in range(n_heads):
            head_attn = attn[:, h, :, :]
            # Diagonal offset -1 captures attention to position t-1
            attn_to_prev = head_attn.diagonal(offset=-1, dim1=-2, dim2=-1)
            # Mean over sequence length, then mean over batch
            attn_to_prev = attn_to_prev.mean(dim=-1).mean(dim=-1).item()
            layer_scores.append(attn_to_prev)
    ...
    for n, batch in tqdm(enumerate(train_loader), total=niters):
        ...
        if n%print_every==0:
            ...
            if progress_measure:
                ...
                print(f"Iteration {n}: Train loss: {loss:.4f}, Train acc: {acc:.4f}, Test loss: {loss_test:.4f}, Test acc: {acc_test:.4f}, IC loss: {loss_ic:.4f}, IC acc: {acc_ic:.4f}, IC2 loss: {loss_ic2:.4f}, IC2 acc: {acc_ic2:.4f}, IW loss: {loss_iw:.4f}, IW acc: {acc_iw:.4f}, prob_icl_labels: {prob_icl_labels_ic:.4f}")
                log_dict = {
                    "Iteration": n, 
                    "Train_Loss": loss, 
                    "Train_Accuracy": acc, 
                    ...
                    "IC_Accuracy": acc_ic, 
                    "IC2_Loss": loss_ic2, 
                    "IC2_Accuracy": acc_ic2, 
                    "IW_Loss": loss_iw, 
                    "IW_Accuracy": acc_iw,
                    "prob_icl_labels": prob_icl_labels_ic
                }
                # Dynamic logging for all layers and heads
                num_layers = len(ih_strengths_ic)
                for l in range(num_layers):
                    num_heads = len(ih_strengths_ic[l])
                    for h in range(num_heads):
                        # Create keys like "IH_strength_L0_H0"
                        suffix = f"_L{l}_H{h}"
                        log_dict[f"IH_strength{suffix}"] = ih_strengths_ic[l][h]
                        log_dict[f"TIIA{suffix}"] = tiias_ic[l][h]
                        log_dict[f"TILA{suffix}"] = tilas_ic[l][h]
                        log_dict[f"TLA{suffix}"] = tlas_ic[l][h]
                        log_dict[f"prev_1_attn{suffix}"] = prev_1_attns_ic[l][h]
                if WANDB:
