...
if __name__ == "__main__":
    ...
    load_encoder_ckpt = bool(int(sys.argv[30])) if len(sys.argv) > 30 else True
    wandb_tag = sys.argv[31] if len(sys.argv) > 31 else None
   ...
   # Initialize wandb
    ...
    if WANDB:
        wandb.init(project="ICL_torch",
                ...
                config={
                    ...
                    "load_encoder_ckpt": load_encoder_ckpt
                },
    ...
    if load_encoder_ckpt:
        Encoder.load_state_dict(torch.load(ckpt_path_enc), strict=True)
        print(f"Loaded encoder checkpoint from {ckpt_path_enc}")
    else:
        print("Using randomly initialized encoder (load_encoder_ckpt=0)")
