...
from model import MLPEncoder,MLPEncoderArgs
import ast
import sys 
import os
...
if __name__ == "__main__":
    ...
    device = torch.device(f"cuda:{int(sys.argv[9])}" if torch.cuda.is_available() else "cpu")
    
    K = int(sys.argv[1])
    eps = float(sys.argv[2])
    input_dim = int(sys.argv[3])
    hidden_sizes = ast.literal_eval(sys.argv[4])
    output_dim = int(sys.argv[5])
    niter = int(sys.argv[6])
    batch_size = int(sys.argv[7])
    save_ckpt = bool(int(sys.argv[8]))
    ...
    prefix = f"./outs_encoder/K{K}_eps{eps}_input_dim{input_dim}_hidden_sizes{hidden_sizes}_output_dim{output_dim}_niter{niter}"
    if WANDB:
        import wandb
    ...
    model_args = MLPEncoderArgs(
        input_dim=input_dim,
        hidden_sizes=hidden_sizes,
        output_dim=output_dim,
        num_classes=K
    )
    model = MLPEncoder(model_args)
    print(f"Model: {model}")
    K1=8192
    L1=L2=32
    D1=64
    _, m1, _, _, mus_class, _, _ = get_mm_mus_label_class(K1=K1,K2=K,L1=L1,L2=L2,D1=D1,D2=input_dim)
